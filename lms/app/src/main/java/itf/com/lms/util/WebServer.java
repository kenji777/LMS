...
import static itf.com.lms.util.Constants.InitialValues.WS_PORT;
...
public class WebServer extends NanoHTTPD {
    ...
    public Response serve(IHTTPSession session) {
        ...
        if ("/test/control/toggle".equals(uri) && "POST".equals(method)) {
            return handleTestControlToggle(session);
        }
        
        // /client_id 엔드포인트 처리
        if ("/client_id".equals(uri)) {
            if ("GET".equals(method)) {
                return createClientIdForm();
            } else if ("POST".equals(method)) {
                return handleClientIdPage(session);
            }
        } else if ("/client_id/lookup".equals(uri) && "GET".equals(method)) {
            return handleClientIdLookup(session);
        }
...
//            }
    ...
    private TestStatusInfo getTestStatusInfo() {
        ...
        try {
            ...
            try {
                Class<?> activityModelClass = Class.forName("itf.com.lms.ActivityModel_0002");
                ...
                try {
                    Class<?> activityModelClass = Class.forName("itf.com.lms.ActivityModel_0001");
                    java.lang.reflect.Method getMainActivityMethod = activityModelClass.getMethod("getMainActivity");
                    activity = getMainActivityMethod.invoke(null);
                } catch (Exception e2) {
                    // ActivityModel_0003 시도
                    try {
                        Class<?> activityModelClass = Class.forName("itf.com.lms.ActivityModel_0003");
                        java.lang.reflect.Method getMainActivityMethod = activityModelClass.getMethod("getMainActivity");
                        activity = getMainActivityMethod.invoke(null);
                    } catch (Exception e3) {
            ...
            if (activity != null) {
                ...
                try {
                    Class<?> activityModelListClass = Class.forName("itf.com.lms.ActivityModelList");
    ...
    private Response handleTestStatus(IHTTPSession session) {
        try {
            ...
            String html = "<!DOCTYPE html>\n" +
                    ...
                    "                            <div class=\"test-list-header-cell\" style=\"flex: 0.05;\">번호</div>\n" +
            ...
            if (status.testItemList.isEmpty()) {
                ...
                for (int i = 0; i < status.testItemList.size(); i++) {
                    ...
                    if (!isBtConnected) {
                        disabledTitle = " title=\"블루투스가 연결되지 않았습니다.\"";
                    ...
                    html += "                            <div class=\"test-item-row " + rowClass + "\">\n" +
                            ...
                            "                                    <button class=\"execute-btn\" onclick=\"executeTestItem('" + item.command + "')\" " + 
                            disabledAttr + disabledTitle + ">실행</button>\n" +
                            "                                </div>\n" +
                            "                            </div>\n";
                }
            }
            ...
            html += "                        </div>\n" +
                    ...
                    "                    \n" +
                    "                    // 테스트 항목 리스트 업데이트\n" +
                    "                    // 제어 버튼 상태 업데이트 (서버에서 받은 timer_running 상태로 동기화)\n" +
                    ...
                    "            .catch(error => {\n" +
                    "                console.error('Error:', error);\n" +
                    "                alert('제어 중 오류가 발생했습니다: ' + (error.message || '알 수 없는 오류'));\n" +
                    "            });\n" +
                    "        }\n" +
                    "        \n" +
        ...
        try {
            ...
            try {
                if (isTimerRunning) {
                    ...
                    Log.i(TAG, "BT message timer stopped via web control");
                    ...
                    Log.i(TAG, "BT message timer started via web control");
                    
                    JSONObject successJson = new JSONObject();
                    try {
                        successJson.put("success", true);
                        successJson.put("message", "타이머가 시작되었습니다.");
                        successJson.put("timer_running", true);
                        String jsonString = successJson.toString();
                        if (jsonString == null || jsonString.isEmpty()) {
                            jsonString = "{\"success\": true, \"message\": \"타이머가 시작되었습니다.\", \"timer_running\": true}";
                        }
                        return newResponse(Response.Status.OK, Constants.HTTP.CONTENT_TYPE_JSON, jsonString);
                    } catch (Exception jsonEx) {
                        Log.e(TAG, "Failed to create success JSON", jsonEx);
                        return newResponse(Response.Status.OK, Constants.HTTP.CONTENT_TYPE_JSON, 
                            "{\"success\": true, \"message\": \"타이머가 시작되었습니다.\", \"timer_running\": true}");
                    }
                }
            } catch (Exception e) {
