...
import itf.com.app.simple_line_test_ovio_new.item.ItemAdapterTestModel;
...
import itf.com.app.simple_line_test_ovio_new.util.PermissionChecker;
...
//                Log.i(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>> testItemArray.length() " + testItemArray.length());
                ...
                if (data_type.equals("model_list")) {
                    for (int i = 0; i < testItemArray.length(); i++) {
                        Log.i(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>> json[" + i + "] " + testItemArray.getString(i));
                    ...
                    Log.i(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>> mapData.size() " + mapData.size());

                    totalTimeCnt = 0;
                    arrTestItems = new String[lstData.size()][mapData.size()];
