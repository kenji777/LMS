...
import itf.com.app.simple_line_test_ovio_new.vo.VoSystemSetting;
...
//                    Toast.makeText(context, VoTestItem.getProcess_no()+" 번 - "+VoTestItem.getProcess_title()+" 입니당! ", Toast.LENGTH_SHORT).show();
        ...
        private void showEmailAppSelectionDialog(Context context, VoSystemSetting voItem) {
            try {
                PackageManager pm = context.getPackageManager();
                if (pm == null) {
                    Toast.makeText(context, "패키지 매니저를 가져올 수 없습니다.", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                dialog.setOnShowListener(d -> {
                    ...
                    dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                        ...
                        if (idx < 0 || idx >= appList.size()) {
                            Toast.makeText(context, "선택된 앱이 없습니다.", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        EmailAppInfo selected = appList.get(idx);
                        if (selected == null || selected.packageName == null || selected.packageName.trim().isEmpty()) {
                            Toast.makeText(context, "선택된 앱 정보가 올바르지 않습니다.", Toast.LENGTH_SHORT).show();
                            return;
                        }
                ...
                Toast.makeText(context, "이메일 앱 목록을 불러오는 중 오류가 발생했습니다.", Toast.LENGTH_SHORT).show();
            }
        }
        ...
        private void saveDefaultEmailApp(Context context, VoSystemSetting voItem, String packageName) {
            try {
                ...
                if (ok) {
                    ...
                    notifyDataSetChanged();
                    Toast.makeText(context, "기본 이메일 앱이 저장되었습니다.", Toast.LENGTH_SHORT).show();
                } else {
                ...
                */
                ...
                dialog.setOnShowListener(d -> {
                    ...
                    dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                        ...
                        if (email.isEmpty()) {
                            Toast.makeText(context, "이메일 주소를 입력해 주세요.", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        // 간단한 형식 체크 (강제는 아님)
...
// clear 버튼 Drawable 준비
                ...
                dialog.setOnShowListener(d -> {
                    ...
                    dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                        ...
                        if (value.isEmpty()) {
                            Toast.makeText(context, "설정값을 입력해 주세요.", Toast.LENGTH_SHORT).show();
                            return;
                        }
        ...
        private void showConfirmSaveDialog(Context context, String title, String message, Runnable onConfirm) {
            try {
                ...
                TextView headerTitle = headerView.findViewById(R.id.tv_dialog_header_title);
                if (headerTitle != null) {
                    headerTitle.setText(title != null ? title : "저장 확인");
                }
                builder.setCustomTitle(headerView);
