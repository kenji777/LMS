...
import static itf.com.app.simple_line_test_ovio_new.util.Constants.InitialValues.WS_PORT;
...
import itf.com.app.simple_line_test_ovio_new.R;
...
public class WebServer extends NanoHTTPD {
    ...
    public Response serve(IHTTPSession session) {
        ...
        if ("/test/control/start".equals(uri) && "POST".equals(method)) {
            ...
            return handleTestControlStart(session);
        }
...
//            }
    ...
    private TestStatusInfo getTestStatusInfo() {
        ...
        try {
            ...
            try {
                Class<?> activityModelClass = Class.forName("itf.com.app.simple_line_test_ovio_new.ActivityModelTestProcess");
            ...
            */
            ...
            if (activity != null) {
                ...
                try {
                    Class<?> activityModelListClass = Class.forName("itf.com.app.simple_line_test_ovio_new.ActivityModelList");
            ...
            try {
                ...
                if (isControlMode && isControlOn) {
                    try {
                        ...
                        Log.i(TAG, "Started waiting for control response: " + command);
                    } catch (Exception e) {
                        Log.e(TAG, "Failed to start waiting for control response", e);
                        // 응답 대기 시작 실패해도 명령은 전송되었으므로 계속 진행
                    }
                }
    ...
    /**
     * 클라이언트 식별자 생성 (IP + sessionId 파라미터 기반)
    ...
    private String generateClientSessionId(IHTTPSession session, String sessionIdParam) {
        try {
            ...
            // sessionId 파라미터가 있으면 사용, 없으면 IP만 사용
            if (sessionIdParam != null && !sessionIdParam.trim().isEmpty()) {
                // return ip + "|" + sessionIdParam.trim();
                return sessionIdParam.trim();
            }
            
            // sessionId 파라미터가 없는 경우 (하위 호환성)
            String userAgent = "";
            try {
            ...
            // IP와 User-Agent를 조합하여 세션 ID 생성 (하위 호환성)
            return ip + "|" + userAgent + "|" + System.identityHashCode(session);
        } catch (Exception e) {
