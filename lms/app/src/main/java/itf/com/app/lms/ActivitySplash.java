...
public class ActivitySplash extends BaseKioskActivity {
    ...
    // MEMORY LEAK FIX: Handler for delayed splash actions with proper cleanup
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(Constants.InitialValues.ACCOUNT_TYPE.equals(Constants.ResultStatus.ACCOUNT_TYPE_CLOUD)) {
            ...
            new LoadSettingInfoTask(this, serverIp).execute();
        }
            ...
            // StringResourceManager 초기화
            StringResourceManager.getInstance().initialize(this);
    ...
    // MEMORY LEAK FIX: Made static with WeakReference to prevent Activity leak
    private static class LoadSettingInfoTask extends AsyncTask<Void, Void, Boolean> {
        private final WeakReference<ActivitySplash> activityRef;
        private final String serverIp;

        LoadSettingInfoTask(ActivitySplash activity, String serverIp) {
        ...
        protected Boolean doInBackground(Void... voids) {
            ActivitySplash activity = activityRef.get();
            ...
            try {
                ...
                LogManager.i(LogManager.LogCategory.SI, TAG, "SettingInfoList.jsp 호출: " + urlStr);
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    LogManager.i(LogManager.LogCategory.SI, TAG, "SettingInfo 응답: " + jsonResponse);
                    ...
                    if (jsonResponse != null && !jsonResponse.trim().isEmpty()) {
                        ...
                        return activity.saveSettingInfoToDatabase(jsonArray);
                    }
                    }
                } else {
                    LogManager.e(LogManager.LogCategory.ER, TAG, "HTTP 응답 코드: " + responseCode);
                }
                
                connection.disconnect();
            } catch (Exception e) {
                LogManager.e(LogManager.LogCategory.ER, TAG, "SettingInfo 다운로드 실패", e);
            }
            return false;
        }
        ...
        protected void onPostExecute(Boolean success) {
            ...
            // 최소 표시 시간 보장을 위해 지연
            // MEMORY LEAK FIX: Use field Handler with proper cleanup instead of anonymous Handler
            activity.schedulePostSplashAction();
        }
    }
    ...
    // MEMORY LEAK FIX: Helper method to schedule post-splash action with cleanup
    private void schedulePostSplashAction() {
    ...
    private boolean saveSettingInfoToDatabase(JSONArray jsonArray) {
        try {
            ...
            LogManager.i(LogManager.LogCategory.SI, TAG, "SettingInfo 저장 완료: " + jsonArray.length() + "건");
            return true;
        } catch (JSONException e) {
            LogManager.e(LogManager.LogCategory.ER, TAG, "JSON 파싱 오류", e);
            return false;
        }
    }
    ...
    private String getJsonString(JSONObject jsonObject, String key) {
        try {
            ...
            LogManager.e(LogManager.LogCategory.ER, TAG, "JSON 키 읽기 실패: " + key, e);
        }
        return "";
    }
    ...
    private boolean checkClientIdRegistration() {
        try {
            ...
            if (settingList != null && !settingList.isEmpty()) {
                LogManager.i(LogManager.LogCategory.SI, TAG, "클라이언트 ID가 이미 등록되어 있습니다.");
                return true;
            }
            LogManager.i(LogManager.LogCategory.SI, TAG, "클라이언트 ID가 등록되지 않았습니다.");
            return true;
        } catch (Exception e) {
            LogManager.e(LogManager.LogCategory.ER, TAG, "클라이언트 ID 확인 실패", e);
            return false;
        }
    }
                    ...
                    if (!input.equals(formattedStr)) {
                        ...
                        etBusinessNumber.post(() -> {
                            try {
                                ...
                                LogManager.e(LogManager.LogCategory.ER, TAG, "커서 위치 설정 오류", e);
                            }
    ...
    private void lookupClientId(String businessNumber) {
        ...
        new LookupClientIdTask(this, serverIp).execute(businessNumber);
    }
     ...
     * MEMORY LEAK FIX: Made static with WeakReference to prevent Activity leak
     */
    private static class LookupClientIdTask extends AsyncTask<String, Void, String> {
        private final WeakReference<ActivitySplash> activityRef;
        private final String serverIp;

        LookupClientIdTask(ActivitySplash activity, String serverIp) {
            this.activityRef = new WeakReference<>(activity);
            this.serverIp = serverIp;
        }
        ...
        protected void onPreExecute() {
            ...
            if (activity.tvErrorMessage != null) {
                activity.tvErrorMessage.setVisibility(View.GONE);
            }
        }
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                LogManager.i(LogManager.LogCategory.SI, TAG, "ApplicationClientInfoList.jsp 호출: " + urlStr);
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    String jsonResponse = sb.toString();
                    LogManager.i(LogManager.LogCategory.SI, TAG, "ApplicationClientInfoList 응답: " + jsonResponse);
...
//                    }
                    ...
                    if (jsonArray != null && jsonArray.length() > 0) {
                        ...
                        if (clientInfo.has(Constants.JsonKeys.CLM_CLIENT_ID)) {
                            ...
                            LogManager.i(LogManager.LogCategory.SI, TAG, "고객 ID 추출 성공: " + clientId);
                            return clientId;
                        }
                    }
                    }
                ...
                LogManager.e(LogManager.LogCategory.ER, TAG, "클라이언트 ID 조회 실패", e);
            }
            return null;
        }
        ...
        protected void onPostExecute(String clientId) {
            ...
            if (clientId != null && !clientId.isEmpty()) {
                // 고객 ID 표시
                if (activity.tvClientId != null) {
                    activity.tvClientId.setText(clientId);
                }
                if (activity.clientIdLayout != null) {
                    activity.clientIdLayout.setVisibility(View.VISIBLE);
                }

                // 등록 버튼 활성화
                if (activity.btnRegister != null) {
                ...
                // 고객 ID를 찾을 수 없음
                activity.showError("해당 사업자 번호로 고객 ID를 찾을 수 없습니다.");
                if (activity.btnRegister != null) {
    ...
    private void registerClientId(String clientId) {
        new RegisterClientIdTask(this).execute(clientId);
    }
     ...
     * MEMORY LEAK FIX: Made static with WeakReference to prevent Activity leak
     */
    private static class RegisterClientIdTask extends AsyncTask<String, Void, Boolean> {
        private final WeakReference<ActivitySplash> activityRef;

        RegisterClientIdTask(ActivitySplash activity) {
            this.activityRef = new WeakReference<>(activity);
        }
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
                ...
                boolean success = TestData.insertSettingInfo(activity, settingData);
                if (success) {
                    LogManager.i(LogManager.LogCategory.SI, TAG, "클라이언트 ID 등록 성공: " + clientId);
                }
                return success;
            } catch (Exception e) {
                LogManager.e(LogManager.LogCategory.ER, TAG, "클라이언트 ID 등록 실패", e);
                return false;
            }
        }
        ...
        protected void onPostExecute(Boolean success) {
            ...
            if (success) {
                ...
                if (activity.clientIdDialog != null && activity.clientIdDialog.isShowing()) {
                    activity.clientIdDialog.dismiss();
                }
                activity.stopPeriodicCheck();
                Intent intent = new Intent(activity, ActivityModelList.class);
                activity.startActivity(intent);
                activity.finish();
            } else {
                activity.showError("등록 중 오류가 발생했습니다.");
            }
        }
    }
    ...
    */
    ...
    protected void onDestroy() {
        ...
        // MEMORY LEAK FIX: Remove all Handler callbacks to prevent leaks
        // Remove splashHandler callbacks
        if (splashHandler != null && splashRunnable != null) {
            splashHandler.removeCallbacks(splashRunnable);
            splashRunnable = null;
        }

        // Remove lookupHandler callbacks
        if (lookupHandler != null && lookupRunnable != null) {
            lookupHandler.removeCallbacks(lookupRunnable);
            lookupRunnable = null;
            lookupHandler = null;
        ...
        if (webView != null) {
            try {
                // MEMORY LEAK FIX: Remove from parent first
                ...
                // MEMORY LEAK FIX: Must call destroy()
                webView.destroy();
                webView = null;
            } catch (Exception e) {
                LogManager.e(LogManager.LogCategory.ER, TAG, "WebView 정리 중 오류", e);
            }
        }
        ...
        if (clientIdDialog != null) {
            try {
                if (clientIdDialog.isShowing()) {
                    clientIdDialog.dismiss();
                }
            } catch (Exception e) {
                LogManager.e(LogManager.LogCategory.ER, TAG, "다이얼로그 닫기 중 오류", e);
            } finally {
                clientIdDialog = null;
            }
        }
